...
\title{Cost-Aware Adaptive Rate Limiting with PPO: Reward Alignment, Budget Utilisation, and Robustness Assessment}
\author{
\IEEEauthorblockN{
Harshal Rudra (MT2025706) \and
Manzil Baruah (MT2025715) \and
Sunil Joshi (MT2025726)
}
}
...
\begin{center}
\href{https://github.com/TheHarshal30/Reinforcement-Learning-Course-Project}{Github Repo}
\end{center}
...
\begin{IEEEkeywords}
reinforcement learning, PPO, API gateway, rate limiting, admission control, cost-aware control, robustness
\end{IEEEkeywords}
...
\begin{axis}[
    ...
    ymax=13,
    ylabel={Throughput},
    symbolic x coords={Base,HP1.0,Thr2.0},
    xtick=data,
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{count_training_curves.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{naive_cost_training_curves.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{cost_expensive_attack_latency.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{azure_duration_histogram_fit.png}
...
\subsection{Temporal Disturbance Evaluation}
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Demand surge}: high-priority demand multiplied by $3$ during SHOCK.
    \item \textbf{Budget cut}: processing budget reduced from $24$ to $8$ during SHOCK.
    \item \textbf{Cost shock}: request costs changed to $\mathcal{U}(8,12)$ during SHOCK.
    \item \textbf{Action corruption}: the chosen action was replaced by a random action with probability $0.30$ during SHOCK.
\end{itemize}

Table~\ref{tab:disturbance_eval} reports averages aggregated across the standard scenarios. The behavior-regularized warm-start PPO was strongest under \texttt{demand\_surge} and \texttt{budget\_cut}, while the heuristic remained strongest under \texttt{cost\_shock} and \texttt{action\_corruption}. This split is informative: learned adaptation helped most when the system had to respond to transient load or temporary capacity loss, whereas the hand-crafted heuristic retained an advantage when shock behavior aligned closely with its built-in priority logic.

\begin{table}[H]
\centering
\caption{Aggregate disturbance robustness across standard scenarios. Shock reward, collapse rate, and recovery rate are averaged across \texttt{low\_load}, \texttt{high\_load}, \texttt{bursty}, and \texttt{oscillating}. The warm-started PPO is best under demand surge and budget cut, while the heuristic remains strongest under cost shock and action corruption.}
\label{tab:disturbance_eval}
\resizebox{\columnwidth}{!}{
\begin{tabular}{llcccc}
\toprule
Disturbance & Policy & Shock Reward & Collapse Rate & Recovery Rate & Robustness Score \\
\midrule
\multirow{3}{*}{demand\_surge}
& PPO-CostOnly & $0.495$ & $0.250$ & $0.138$ & $0.549$ \\
& PPO-WarmStart-Reg ($\lambda=0.1$) & $0.599$ & $0.250$ & $0.312$ & $0.618$ \\
& CostAwarePriority & $0.653$ & $0.263$ & $0.238$ & $0.599$ \\
\midrule
\multirow{3}{*}{budget\_cut}
& PPO-CostOnly & $-0.441$ & $0.900$ & $0.000$ & $0.122$ \\
& PPO-WarmStart-Reg ($\lambda=0.1$) & $-0.118$ & $0.938$ & $0.000$ & $0.141$ \\
& CostAwarePriority & $-0.971$ & $0.887$ & $0.000$ & $0.071$ \\
\midrule
\multirow{3}{*}{cost\_shock}
& PPO-CostOnly & $-0.318$ & $0.875$ & $0.000$ & $0.087$ \\
& PPO-WarmStart-Reg ($\lambda=0.1$) & $-0.299$ & $0.838$ & $0.000$ & $0.135$ \\
& CostAwarePriority & $-0.152$ & $0.787$ & $0.000$ & $0.222$ \\
\midrule
\multirow{3}{*}{action\_corruption}
& PPO-CostOnly & $0.685$ & $0.138$ & $0.275$ & $0.665$ \\
